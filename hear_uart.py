...
HEART_RATE_TLV_STRUCT = struct.Struct("<5f4H")
...
@dataclass(slots=True)
class HeartRateTlv1001:
    # Estimated heart rate in beats per minute; this is the main value shown to the user.
    heart_rate_bpm: float
    # Same estimate expressed in hertz; useful when matching embedded spectral peaks.
    heart_rate_hz: float
    # Confidence score of the current estimate; lower values usually indicate unstable tracking.
    confidence: float
    # Effective slow-time sample rate used by the embedded estimator after frame averaging/decimation.
    sample_rate_hz: float
    # Distance of the selected chest gate in meters; used to confirm which target range is tracked.
    range_meters: float
    # Selected range FFT bin index; links the result back to the chosen range gate in the radar cube.
    selected_range_bin: int
    # Number of slow-time samples in the current estimation window; determines latency and frequency resolution.
    window_length: int
    # Non-zero means the current heart-rate estimate is marked valid by the embedded algorithm.
    valid: int

    @classmethod
    def from_payload(cls, payload: bytes) -> "HeartRateTlv1001":
        (
            heart_rate_bpm,
            heart_rate_hz,
            confidence,
            ...
            valid,
            _reserved,
        ) = HEART_RATE_TLV_STRUCT.unpack(payload)
    ...
    def as_csv_row(self, header: FrameHeader) -> dict[str, float | int]:
        return {
            ...
            "selected_range_bin": self.selected_range_bin,
...
def validate_frame_header(header: FrameHeader) -> str | None:
    if header.total_packet_len < HEADER_LEN:
        return f"total_packet_len={header.total_packet_len} smaller than header"
    if header.total_packet_len > MAX_TOTAL_PACKET_LEN:
        return f"total_packet_len={header.total_packet_len} exceeds max={MAX_TOTAL_PACKET_LEN}"
    if (header.total_packet_len % 32) != 0:
        return f"total_packet_len={header.total_packet_len} is not 32-byte aligned"
...
def describe_rx_buffer(rx_buffer: bytearray) -> str:
    ...
    if have_len < header.total_packet_len:
        return (
            f"rxBuf={len(rx_buffer)} magicAt={start} "
            f"frame={header.frame_number} total={header.total_packet_len} have={have_len}"
        )

    return (
        f"rxBuf={len(rx_buffer)} magicAt={start} "
        f"frame={header.frame_number} total={header.total_packet_len} have={have_len} state=ready"
    )
...
def read_one_frame(data_ser: serial.Serial, rx_buffer: bytearray) -> bytes | None:
    ...
    if len(rx_buffer) < header.total_packet_len:
        return None

    frame = bytes(rx_buffer[:header.total_packet_len])
    del rx_buffer[:header.total_packet_len]
    return frame


def parse_tlvs(frame: bytes) -> tuple[FrameHeader, list[tuple[int, bytes]], str | None]:
    ...
    for _ in range(header.num_tlvs):
        ...
        if offset + tlv_length > len(frame):
            error = f"truncated TLV payload type={tlv_type} len={tlv_length} offset={offset}"
            break
...
def format_heart_rate_line(header: FrameHeader, heart_rate: HeartRateTlv1001) -> str:
    return (
        f"[HR] frame={header.frame_number} subFrame={header.subframe_number} "
        f"valid={heart_rate.valid} bpm={heart_rate.heart_rate_bpm:.2f} "
        f"hz={heart_rate.heart_rate_hz:.3f} conf={heart_rate.confidence:.3f} "
        f"gate={heart_rate.selected_range_bin} range={heart_rate.range_meters:.3f} m "
        f"fs={heart_rate.sample_rate_hz:.3f} Hz win={heart_rate.window_length}"
    )
...
def monitor_data(
    ...
    while True:
        ...
        if tlv_error is not None:
            print(f"[TLV] frame={header.frame_number} subFrame={header.subframe_number} {tlv_error}")

        for tlv_type, payload in tlvs:
            if tlv_type != MMWDEMO_OUTPUT_MSG_HEART_RATE:
                continue

            if len(payload) != HEART_RATE_TLV_STRUCT.size:
                print(
            ...
            heart_rate = HeartRateTlv1001.from_payload(payload)
            heart_rate_found = True
            heart_rate_writer.writerow(heart_rate.as_csv_row(header))
            heart_rate_csv.flush()
            print(format_heart_rate_line(header, heart_rate))
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Send try.cfg on COM4 and parse heart-rate TLV 1001 from COM3."
    )
    parser.add_argument(
        "--cfg-port",
